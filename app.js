...
/* ─── THEME ─── */
const C = {
  bg:'#06101e', card:'#0d1b2e', primary:'#0ea5e9', sec:'#0369a1',
  accent:'#38bdf8', text:'#e2e8f0', muted:'#94a3b8',
  green:'#10b981', amber:'#f59e0b', red:'#f43f5e', gray:'#64748b', blue:'#818cf8',
  grid:'rgba(14,165,233,0.08)', tick:'#64748b',
  PALETTE:['#0ea5e9','#10b981','#f59e0b','#f43f5e','#8b5cf6',
           '#ec4899','#14b8a6','#f97316','#06b6d4','#84cc16',
           '#a855f7','#22d3ee','#fb923c','#34d399']
};
  ...
  _base() {
    return {
      ...
      plugins:{
        ...
        tooltip:{
          backgroundColor:'rgba(6,16,30,0.97)', borderColor:'rgba(14,165,233,0.40)', borderWidth:1,
...
async function initSleepersPage() {
  ...
  function renderView(rows) {
    ...
    } else if (ogRows.length===1) {
      // Single ongoing tender → gauge donut
      const t=ogRows[0];
      const totalQ=t['Total Tender Qty']||1;
      const casted=t['Total Qty Casted']||0;
      const balance=Math.max(0,t['Balance To Cast']||(totalQ-casted));
      const pct=Math.min(100,casted/totalQ*100);
      Charts.donut('slp-progress',['Casted','Balance to Cast'],[casted,balance],{
        cutout:'72%',centerText:pct.toFixed(0)+'%',centerSub:'Complete',
        centerColor:pct>=75?C.green:pct>=40?C.amber:C.red,unit:' pcs'
      });
    } else if (ogRows.length>1) {
      const og=ogRows.slice(0,12);
      const pcts=og.map(r=>{ const t=r['Total Tender Qty']||1,c=r['Total Qty Casted']||0; return Math.min(100,c/t*100); });
    ...
    // Chart 2 — For single ongoing: Casted vs Balance donut; otherwise site distribution
    if (ogRows.length===1&&selStatus!=='Completed') {
      const t=ogRows[0];
      const totalQ=t['Total Tender Qty']||0;
      const casted=t['Total Qty Casted']||0;
      const balance=Math.max(0,t['Balance To Cast']||(totalQ-casted));
      Charts.donut('slp-donut',['Casted','Balance to Cast'],[casted,balance],{unit:' pcs'});
    } else {
      const bySite={};
      rows.forEach(r=>{ const s=r['Site Name']||'Unknown'; bySite[s]=(bySite[s]||0)+1; });
